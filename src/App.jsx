    ...
    <div>
      <section className='min-h-screen bg-gray-950 flex justify-center items-center text-white'>
        ...
        <div className='flex justify-between mb-10 px-5'>
          <button onClick={()=> setShowFormBalance(true)} disabled ={initBalance > 0 ? true : false} >Add Balance</button>
          <button onClick={() => setShowFormExpense(true)} disabled = {getTotalExpenses >= initBalance  ? true : false}>Add Expenses</button>
        </div>
        {showFormBalance && (
          <FormBalance setShowFormBalance={setShowFormBalance} setInitBalance={setInitBalance}/>)   }
        {showFormExpense && (<FormExpense setShowFormExpense={setShowFormExpense} listExpenses={listExpenses} setListExpenses={setListExpenses} />)}
